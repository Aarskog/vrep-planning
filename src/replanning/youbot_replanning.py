...
class youbot_replan:
	def __init__(self,world_size,robot_start,goal,obstacles=None,path=None):
		...
		self.path = path
		self.make_pddl_problem(robot_start,goal,obstacles,world_size,path)
	...
	def make_pddl_problem(self,robot_start,goal,obstacles,world_size,path):
		...
		if obstacles:
			for obstacle in obstacles:
				lines.append(obstacle.strips_name)
		...
		if obstacles:
			obstacle_num = 1
			for obstacle in obstacles:
				obstacle_name = obstacle.strips_name
...
class Obstacle:
	def __init__(self,pos,strips_name,moveable=True):
		self.pos = pos
		self.moveable = moveable
		self.discovered = False
		self.strips_name = strips_name
		if moveable:
			self.name = ' O '
		else:
			self.name = ' | '
